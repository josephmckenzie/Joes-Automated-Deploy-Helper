...
Create_AWS_Stack(){
echo "Please enter a name for your S3 Bucket"
read S3
...
#									fi
          ...
          do
            case $AmazonOpt in
              "AWS Config")
							  echo "Do you have your AWS access and secret keys stored on this computer? (Y/n)"
								read CONFIGKEYS
              ...
              "S3 Bucket")
                ...
                ;;
							"Create a SQS/Lambda Stack")
									...
									echo "You need a S3 Bucket to continue, Do you currently have one? (Y/n)"
									read -r BUCKETANSWER
									if [[ "$BUCKETANSWER" == [Yy] ]]; then
											echo "Please enter the name of your bucket."
											read -r S3BUCKETNAME
											echo "Please enter a name for the stack. (I Recommend keping the same as your bucket for consistency)"
											read -r STACKNAME	
											aws cloudformation package --template-file cloudformation.yml --s3-bucket "$S3BUCKETNAME" --output-template-file output.yml && aws cloudformation deploy --template-file output.yml --stack-name "$STACKNAME" --capabilities CAPABILITY_IAM
									else 
										echo "First We need to a S3 Bucket"
